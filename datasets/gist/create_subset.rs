/// Create subset datasets from GIST-1M for fast development iteration
/// 
/// Usage:
///   cargo run --release --bin create_gist_subset -- 100000
///   cargo run --release --bin create_gist_subset -- 50000
...
fn main() -> std::io::Result<()> {
    ...
    if args.len() != 2 {
        ...
        eprintln!("Example: {} 100000", args[0]);
        eprintln!("Creates subset with first N vectors from GIST-1M");
        std::process::exit(1);
    }
    ...
    println!("╔═══════════════════════════════════════════════════════════════╗");
    println!("║           GIST Subset Creator                                 ║");
    println!("╚═══════════════════════════════════════════════════════════════╝");
    println!();
    println!("Creating subset with {} vectors (960 dimensions)...", subset_size);
    println!();
    ...
    // Queries stay the same (use all 1K)
    println!();
    println!("Note: Using full query set (1K queries)");
