...
use crate::index::hierarchical_with_rabitq::{ClusteredIndexWithRaBitQ, SearchStatsWithRaBitQ, ClusterNodeWithRaBitQ};
use crate::index::hierarchical::ClusterNode;
use std::collections::HashSet;
use std::fs::File;
use std::io::{BufWriter, Write};

/// Trait for nodes in the hierarchical index
pub trait IndexNode {
    fn vector_indices(&self) -> &[usize];
    fn children(&self) -> &[usize];
    fn id(&self) -> usize;
}

impl IndexNode for ClusterNode {
    fn vector_indices(&self) -> &[usize] {
        &self.vector_indices
    }
    fn children(&self) -> &[usize] {
...
impl IndexNode for ClusterNodeWithRaBitQ {
    fn vector_indices(&self) -> &[usize] {
        &self.vector_indices
    }
    fn children(&self) -> &[usize] {
...
/// Trait for hierarchical indices
pub trait HierarchicalIndex {
    type Node: IndexNode;
    fn nodes(&self) -> &[Self::Node];
}

impl HierarchicalIndex for ClusteredIndex {
...
impl HierarchicalIndex for ClusteredIndexWithRaBitQ {
    type Node = ClusterNodeWithRaBitQ;
    fn nodes(&self) -> &[Self::Node] {
        &self.nodes
    }
}

/// Trait for search statistics
pub trait SearchStatistics {
    fn leaves_searched_ids(&self) -> &[usize];
}

impl SearchStatistics for SearchStats {
    fn leaves_searched_ids(&self) -> &[usize] {
        &self.leaves_searched_ids
    }
}

impl SearchStatistics for SearchStatsWithRaBitQ {
    fn leaves_searched_ids(&self) -> &[usize] {
        &self.leaves_searched_ids
    }
}
...
pub fn visualize_vector_space<I, S>(
    index: &I,
    all_vectors: &[Vec<f32>],
    query: &[f32],
    ...
    S: SearchStatistics,
{
    ...
    let searched_set: HashSet<usize> = search_stats.leaves_searched_ids()
        .iter()
        .flat_map(|&leaf_id| {
            index.nodes()[leaf_id].vector_indices().to_vec()
        })
        .collect();
...
pub fn visualize_tree_structure<I, S>(
    index: &I,
    search_stats: &S,
    ground_truth_indices: &[usize],
    output_file: &str,
...
pub fn print_coverage_report<I, S>(
    index: &I,
    ground_truth_indices: &[usize],
    search_stats: &S,
) 
